...
public:
  ...
  double average_n_cutsequences(double &proper_average);
  double average_n_cutsequences_stats(
  ...
  int random_cost_bias_sampled_cost(int x1, int y1, int x2, int y2,
                                    vector<NavPoint> current_poses,
                                    Pos_list &poses);

  int visibility_sampled_cost(Pos_list &poses, Segment &l1);
  int sampled_cost_split(Segment &to_line1, Segment &to_line2, int left_o,
                         int right_o, int new_o, int &b_l, int &b_r);

  int move_between_two_lines(Point left_start, Point right_start,
                             Point left_end, Point right_end, int left_o,
                             int right_o, int &last_c,
  ...
  inline bool are_adjacent(int i, int j) {
    int val = abs(i - j);
    if (val < 2 || val >= _n - 1) {
      return true;
    }
...
private:
  ...
  CutSequence *new_cutsequence_from(ChoiceSet *cs, int j, int b_l, int b_r,
