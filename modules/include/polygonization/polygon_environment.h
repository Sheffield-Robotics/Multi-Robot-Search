...
public:
  ...
  std::vector<Segment_Visibility_Graph::vertex> seg_vis_graph_type1_vertices;
  std::vector<Segment_Visibility_Graph::vertex> seg_vis_graph_type2_vertices;
  ...
  bool vertex_is_reflexive(int i, VisiLibity::Visibility_Polygon &v_poly);
  int get_next_index(int i);
  int get_prev_index(int i);

  KERNEL::Point_2
  get_point_of_segment_on_segment(std::list<KERNEL::Segment_2> l, int k);

  KERNEL::Point_2 get_point_of_segment_on_segment(KERNEL::Segment_2 s, int k,
                                                  bool &success);
  ...
  add_edge_to_visibility_graph(int i, int type_i, int j, int type_j, double d,
  ...
  std::list<KERNEL::Segment_2> get_shortest_path(KERNEL::Point_2 v, int j,
                                                 int k, double &final_dist);

  std::list<KERNEL::Segment_2> get_shortest_path(int i, int j,
                                                 double &final_dist);

  std::vector<std::vector<std::list<KERNEL::Segment_2>>> *
      get_shortest_path_cache;
  std::vector<std::vector<double>> *get_shortest_path_distance_cache;
  std::vector<std::vector<bool>> *get_shortest_path_cache_filled;
  ...
  bool get_split_distances(int i, int j, int k, double &d1, double &d2,
                           double r = 1);
  int get_split_cost(int i, int j, int k, double r = 1);
  std::list<KERNEL::Segment_2> shortest_split_cost(int i, int j, int k,
                                                   double &cost);
  std::list<KERNEL::Segment_2> shortest_split_costs(int i, int j, int k,
                                                    double &cost1,
                                                    double &cost2,
  ...
  split_at_point(KERNEL::Point_2 v, int i, int j, int k, double &cost1,
                 double &cost2, int &split_point_index,
                 std::list<KERNEL::Segment_2> &ll1,
                 std::list<KERNEL::Segment_2> &ll2);

  KERNEL::Point_2 find_same_angle_point(KERNEL::Point_2 a1, KERNEL::Point_2 a2,
                                        KERNEL::Line_2 l);

  KERNEL::Segment_2 get_segment_to_k(std::list<KERNEL::Segment_2> &l, int k);
  ...
  KERNEL::Point_2 closest_point_on_line_to(KERNEL::Line_2 l, KERNEL::Point_2 p);

  double shortest_distance_between(KERNEL::Segment_2 s, KERNEL::Point_2 p,
                                   KERNEL::Point_2 &closest_point);

  Segment_Visibility_Graph::vertex get_segment_visibility_vertex(int i,
                                                                 int type_i);

  KERNEL::Point_2 Point_2_from_poly_vertex(VisiLibity::Point &p);
  ...
  bool is_in_polygon(Segment s, Polygon *poly, Polygon::Vertex_iterator &it,
                     int &source_to_target);
  bool get_connections_segments(Segment &shortest_s, Segment &master_s,
                                Segment &poly_s,
                                Polygon::Vertex_iterator &poly_i,
                                Polygon::Vertex_iterator &master_i, int poly_id,
                                int goal, Polygon *poly);

  double shortest_path_distance_between(double x1, double y1, double x2,
                                        double y2);

  bool is_sequence_split(int i, int j, int k);
  bool is_sequential(int i, int j);

  void process_visibility_polygon(int main_segment_index, KERNEL::Point_2 v,
                                  VisiLibity::Visibility_Polygon &v_poly,
                                  bool special_run = false);
