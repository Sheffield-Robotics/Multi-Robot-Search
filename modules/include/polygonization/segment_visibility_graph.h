...
public:
  ...
  public:
    int segment_index;
    int type; // vertices are either type 1 (corresponds to segment/edge in a
              // polygon) or type 2 (corresponds to a vertex in a polygon )
    bool reflexive;
    double p_x;
    double p_y;
    double seg_x;
    ...
    seg_vertex(int i, int t, double x, double y, double x1, double y1,
               double x2, double y2)
        : segment_index(i), type(t), p_x(x), p_y(y), seg_x(x1), seg_y(y1),
          seg_x2(x2), seg_y2(y2) {
      reflexive = false;
    };
  ...
  public:
    ...
    seg_edge(int i, int j, double d)
        : segment_index(i), segment_index2(j), distance(d) {
      point_is_set = false;
    };
  ...
  typedef boost::adjacency_list<boost::listS, boost::vecS, boost::undirectedS,
                                seg_vertex, seg_edge> mygraph_t;
  typedef boost::property_map<mygraph_t, double seg_edge::*>::type WeightMap;
  ...
  std::list<vertex> get_shortest_path(Segment_Visibility_Graph::vertex start,
                                      Segment_Visibility_Graph::vertex end);

  Segment_Visibility_Graph(int n_v, int n_e, float *edge_weights,
                           edge *edge_array);
  ...
  edge_descriptor add_edge(vertex v1, vertex v2, double x, double y, double x2,
                           double y2, double d);
  std::pair<Segment_Visibility_Graph::edge_descriptor, bool>
  get_edge_shortest(vertex v, vertex w);
  ...
  public:
    astar_goal_visitor(Vertex goal, Vertex start, ColorMap *colormap)
        : m_goal(goal), m_start(start), m_colormap(colormap) {}
