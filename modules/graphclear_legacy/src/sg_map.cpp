	...
	for ( int i = 0 ; i < n_polygons; ++i ) {
		...
		for ( int j = 1; j < poly_boundary_simple[i].size(); ++j ) {
			...
			if ( a_site.is_defined()
                && bound_check(int(P1.x()),int(P1.y()))
                && bound_check(int(P2.x()),int(P2.y())) )
...
void sg_map::process_CGAL_voronoi_diagram() {
	...
	for ( ; edge_it != edge_it_end; ++edge_it ) {
		if ( edge_it->is_segment() ) {
			...
			if ( bound_check(int(VP1.x()), int(VP1.y())) 
              && occ( int(VP1.x()), int(VP1.y()) ) == 0 
    ...
    cout << " - visit vertices in CGAL Voronoi Diagram done" << endl;
	char filename[100];
...
sg_vertex_d sg_map::visit_VD_vertex( VD_Vertex vert ) {
    ...
    if ( ! bound_check(vert.point().x(), vert.point().y() ) ) {
        sg_vertex_d error_v;
