...
void ChoiceTree::init_choice_tree() {
  ...
  n_skipped = skipped;
  n_fully_skipped = fully_skipped;
  n_not_skipped = not_skipped;
...
int ChoiceTree::update_costs(CutSequence *cut_seq, int first_o) {
  ...
  while (o_it != o_sequence.end()) {
    ...
    if (map_it != blocking_lines_parent.end()) {
      ...
      if (DEBUG_TRAJECTORY_CREATION >= 1) {
        M_INFO2("pare %d of %d left? %d", parent_id, _sg_index,
                map_it2->second);
      }
    ...
    int ext_cost = _e->get_shortest_extension(left_o, right_o, new_o, l1, l2);
    if (DEBUG_TRAJECTORY_CREATION >= 1) {
    ...
    if (Params::g_visibility_line_sampling_type == 1) {
      ext_cost = sampled_cost_split(l1, l2, left_o, right_o, new_o,
                                    b_l_vissample, b_r_vissample);
    }
    ...
    int b_removed = _e->get_shortest_line_inside_cost(left_o, right_o);
...
int ChoiceTree::computeElevationMapBorderCoverageCost(int x1, int y1, int x2,
                                                      int y2) {
  ...
  while (get_next_point(&line)) {
    ...
    if (!peri.verify(x, y))
      continue;
    if (_map->getCellsMM()[x][y].getClass() == HeightCell::ELC_FLAT_GROUND) {
...
int ChoiceTree::random_sampled_cost(int x1, int y1, int x2, int y2,
                                    Pos_list &poses) {
  HeightMap *_map = _v->getHeightMap();
  deque<Perimeter::Cell> cells;
  Perimeter peri(_map, _v, 0, 0, 0);
...
int ChoiceTree::random_cost_bias_sampled_cost(int x1, int y1, int x2, int y2,
                                              vector<NavPoint> current_poses,
                                              Pos_list &poses) {
  HeightMap *_map = _v->getHeightMap();
  deque<Perimeter::Cell> cells;
...
int ChoiceTree::sampled_cost_split(Segment &to_line1, Segment &to_line2,
                                   int left_o, int right_o, int new_o, int &b_l,
                                   int &b_r) {
  ...
  if (move_to_triangle_base) {
    ...
    int c = move_between_two_lines(left_block_point, right_block_point,
                                   left_split_point, right_split_point, left_o,
                                   right_o, last_c);
    if (c > max_c) {
      max_c = c;
    }
  ...
  if (!left_no_block && !right_no_block) {
    ...
    int steps =
        ceil(sqrt(CGAL::to_double(squared_distance(base_point, split_point))) /
  ...
  if (true || new_block_l.target() != new_block_l.source()) {
    ...
    max_b_l = move_between_two_lines(left_split_point, split_point, to_l, to_r,
                                     left_o, new_o, b_l, start_recording);
  } else {
    b_l = 0;
  }
  ...
  if (true || new_block_r.target() != new_block_r.source()) {
    ...
    if (_e->is_on_line(right_o, new_o, dumm) == right_o) {
      to_l = new_block_r.target();
      to_r = new_block_r.source();
    } else {
      to_l = new_block_r.source();
      to_r = new_block_r.target();
    }
    max_b_r = move_between_two_lines(split_point, right_split_point, to_l, to_r,
                                     new_o, right_o, b_r, start_recording);
  } else {
    b_r = 0;
...
int ChoiceTree::move_between_two_lines(Point left_start, Point right_start,
                                       ...
                                       int left_o, int right_o, int &last_c,
                                       int start_recording) {

  double left_dist = CGAL::to_double(squared_distance(left_end, left_start));
  double right_dist = CGAL::to_double(squared_distance(right_end, right_start));
  double move_on_dist;
  ...
  int steps = ceil(sqrt(move_on_dist) / Params::g_trajectory_discretization) +
              1; // TODO: check the +1
  if (steps == 0) {
...
void ChoiceTree::record_poses(int o1, int o2, Pos_list &poses, int at) {
  ...
  if (at != -1 && at < int(all_lines_in_time.size())) {
    if (DEBUG_RECORD_POSES > 2) {
...
int ChoiceTree::visibility_sampled_cost(Pos_list &poses, Segment &l1) {
  ...
  if (Params::g_visibility_line_sampling_type_method == 1) {
    return _v->get_visibility_line_cost(floor(CGAL::to_double(l1.source().x())),
                                        floor(CGAL::to_double(l1.source().y())),
                                        floor(CGAL::to_double(l1.target().x())),
                                        floor(CGAL::to_double(l1.target().y())),
                                        poses);
    ...
    return this->random_cost_bias_sampled_cost(
        ...
        floor(CGAL::to_double(l1.source().y())),
        floor(CGAL::to_double(l1.target().x())),
        floor(CGAL::to_double(l1.target().y())), current_nav_points, poses);
  } else if (Params::g_visibility_line_sampling_type_method == 3) {
    return this->random_sampled_cost(floor(CGAL::to_double(l1.source().x())),
                                     floor(CGAL::to_double(l1.source().y())),
                                     floor(CGAL::to_double(l1.target().x())),
                                     floor(CGAL::to_double(l1.target().y())),
                                     poses);
...
void ChoiceTree::load_from_file(std::string filename) {
  ...
  std::cout << " average_n_cutsequences proper " << proper << std::endl;
...
double ChoiceTree::average_n_cutsequences(double &proper_average) {
  // std::cout << "Getting average number of cut sequences " << std::endl;
  double avg2 = 0;
  double pavg2 = 0;
  double ll = 0;
  int n_cut_seqs_max = 0;
  for (int k = 1; k < _n; ++k) {
    double avg = 0;
    double pavg = 0;
    double l = 0;
    for (int i = 1; i <= _n; ++i) {
      ...
      avg += n_cut_seqs;
      if (n_cut_seqs > n_cut_seqs_max) {
        n_cut_seqs_max = n_cut_seqs;
      }
    }
    avg = avg / _n;
    if (l > 0) {
...
double ChoiceTree::average_n_cutsequences_stats(double &proper_average, std::vector<int>& n_choicesets_with_n_cutsequences) {
  // std::cout << "Getting average number of cut sequences " << std::endl;
  double avg2 = 0;
  double pavg2 = 0;
  double ll = 0;
  ...
  for (int j = 1; j <= k; j++) {
    ...
    for (int l_i = 0; l_i < cs_l->cut_sequences_size(); l_i++) {
      ...
      for (int r_i = 0; r_i < cs_r->cut_sequences_size(); r_i++) {
        ...
        CutSequence *new_cs;
        new_cs = new_cutsequence_from(cs, j, cs_l->get_b(), cs_r->get_b(),
                                      cut_seq_l, cut_seq_r);
    ...
    if (DEBUG_CHOICETREE >= 1) {
      std::cout << " have " << cs->cut_sequences_size() << " cut sequences"
                << std::endl;
    }
    if (DEBUG_CHOICETREE >= 2) {
...
CutSequence *ChoiceTree::new_cutsequence_from(ChoiceSet *cs, int j, int b_l,
                                              int b_r, CutSequence *cut_seq_l,
  ...
  if (DEBUG_CHOICETREE >= 3) {
    std::cout << "      1) ADDING base obstacle index"
              << cs->get_obstacle_for_choice(j) << std::endl;
  }
  ...
  while (building_cutsequence) {
    ...
    if (l_done || (!r_done && r_i->get_rho() <= l_i->get_rho())) {
      ...
      r_o_i++;
    } else if (r_done || (!l_done && l_i->get_rho() <= r_i->get_rho())) {
    ...
    if (mu_current > mu_max) {
      if (candidate_cut_exists) {
        ...
        if (DEBUG_CHOICETREE >= 3) {
          std::cout << "      3) New cut " << candidate_cut.get_mu() << ","
...
int ChoiceTree::get_number_of_obstacles() { return _n; }
}