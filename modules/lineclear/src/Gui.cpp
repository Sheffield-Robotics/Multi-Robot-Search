...
void Gui::newFile() { infoLabel->setText(tr("Invoked <b>File|New</b>")); }

void Gui::open() { infoLabel->setText(tr("Invoked <b>File|Open</b>")); }

void Gui::save() { infoLabel->setText(tr("Invoked <b>File|Save</b>")); }

void Gui::print() { infoLabel->setText(tr("Invoked <b>File|Print</b>")); }

void Gui::undo() { infoLabel->setText(tr("Invoked <b>Edit|Undo</b>")); }
...
void Gui::redo() {
  ...
  pgi->setVerticesPen(
      QPen(Qt::yellow, 4, Qt::SolidLine, Qt::RoundCap, Qt::RoundJoin));
  pgi->setEdgesPen(
...
void Gui::cut() {
  ...
  std::cout << " get_shortest_line_intersected(" << o1 << "," << o2
            << std::endl;
  ...
  for (i = intR.begin(); i != intR.end(); i++) {
    ...
    minkgi =
        new CGAL::Qt::PolygonWithHolesGraphicsItem<Polygon_wh>(poly_with_holes);
    minkgi->setVerticesPen(
        QPen(Qt::red, 8, Qt::SolidLine, Qt::RoundCap, Qt::RoundJoin));
    minkgi->setEdgesPen(
        QPen(Qt::blue, 4, Qt::SolidLine, Qt::RoundCap, Qt::RoundJoin));
...
void Gui::italic() {
  ...
  if (cleared_obstacles.size() > 1) {
    ...
    Segment l1, l2, l3, l4;
    std::cout << " extending  " << next_smaller_obstacle << " and "
              << next_larger_obstacle << " onto " << new_obstacle << std::endl;
    int ext_cost = e->get_shortest_extension(
        next_smaller_obstacle, next_larger_obstacle, new_obstacle, l1, l2);
    ...
    std::cout << " current_blocking_cost= " << current_blocking_cost
              << std::endl;
    ...
    if (l4.target() != l4.source()) {
      ...
      std::pair<int, int> block_between(next_smaller_obstacle, new_obstacle);
...
void Gui::aboutQt() { infoLabel->setText(tr("Invoked <b>Help|About Qt</b>")); }
...
void Gui::createActions() {
  ...
  connect(setParagraphSpacingAct, SIGNAL(triggered()), this,
