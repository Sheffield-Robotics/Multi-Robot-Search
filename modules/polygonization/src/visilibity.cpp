...
#include "polygonization/visilibity.hpp"  //VisiLibity header
...
namespace VisiLibity
{
 ...
 Point Point::projection_onto(const Polyline& polyline_temp) const
  ...
  int Point::index_of_projection_onto_boundary_of(const Polygon& polygon_temp) const
  {
    assert( *this == *this
    ...
    double running_min = distance(*this, running_projection);    
    Point point_temp;
    int last_projection_on_vertex_i = -1;
    for(unsigned i=0; i<=polygon_temp.n()-1; i++){
      ...
      if( distance(*this, point_temp) < running_min ){
        running_projection = point_temp;
        running_min = distance(*this, running_projection);
        last_projection_on_vertex_i = i;
      }
    }
    return last_projection_on_vertex_i;
  }
  
  int Point::index_of_projection_onto_boundary_of(const Environment&
                                           environment_temp) const
  {
    assert( *this == *this
            and environment_temp.n() > 0 );    
    return index_of_projection_onto_boundary_of(environment_temp.outer_boundary_);
  }
