...
Polygon_Environment::Polygon_Environment(int n) {
  ...
  CGAL::random_polygon_2(n, std::back_inserter(*polygon), Point_generator(400));
...
void Polygon_Environment::construct(Alpha_shape *alphaShape, float epsilon) {
  ...
  for (unsigned int i = 0; i < this->size(); i++) {
    area_list.push_back(
        std::pair<int, double>(i, fabs(CGAL::to_double(this->at(i).area()))));
  }
  area_list.sort(pair_comparison);
  list_i = area_list.begin();
  ...
  if (this->size() == 1) {
    ...
    while (list_i != distance_list.end()) {
      ...
      // if ( list_i->first == 118 )
      //    break;
      add_to_master(&this->at(list_i->first), list_i->first,
                    second_largest_poly_i);
      list_i++;
    }
...
void Polygon_Environment::process_master_polygon() {
  ...
  for (; it != master_polygon->vertices_end();) {
    VisiLibity::Point poi(CGAL::to_double((*it).x()),
                          CGAL::to_double((*it).y()));
    ...
    Segment_Visibility_Graph::seg_vertex a_vertex(
        i, 1, (next_x + poi.x()) / 2, (next_y + poi.y()) / 2, poi.x(), poi.y(),
        next_x, next_y);
    v_desc = seg_vis_graph->add_vertex(a_vertex);
    seg_vis_graph_type1_vertices.push_back(v_desc);
    Segment_Visibility_Graph::seg_vertex b_vertex(i, 2, poi.x(), poi.y());
  ...
  Segment_Visibility_Graph::seg_vertex a_special_vertex(n_segs, 2, 0, 0, 0, 0,
                                                        0, 0);
  ...
  for (int ii = 0; ii < env_outer_poly->n(); ii++) {
    ...
    KERNEL::Point_2 v(this->get_visi_vertex(ii).x(),
                      this->get_visi_vertex(ii).y());
  ...
  for (int i = 0; i < this->seg_vis_graph_type2_vertices.size(); i++) {
    ...
    if ((*g)[v1].reflexive && (*g)[v2].reflexive) {
      ...
      double dis = pow((*g)[v1].p_x - (*g)[v2].p_x, 2) +
                   pow((*g)[v1].p_y - (*g)[v2].p_y, 2);
      dis = sqrt(dis);
      this->add_edge_to_visibility_graph(i, 2, next_i, 2, dis, (*g)[v1].p_x,
                                         (*g)[v1].p_y, (*g)[v2].p_x,
                                         (*g)[v2].p_y);
    }
  }
...
bool Polygon_Environment::check_path_cache(int i, int j) {
  if (i > j) {
    std::swap(i, j);
  };
...
std::list<KERNEL::Segment_2> Polygon_Environment::get_path_cache(int i, int j) {
  if (i > j) {
    std::swap(i, j);
  };
  if ((*get_shortest_path_cache_filled)[i][j]) {
...
double Polygon_Environment::get_path_distance(int i, int j) {
  if (i > j) {
    std::swap(i, j);
  };
  if ((*get_shortest_path_cache_filled)[i][j]) {
...
void Polygon_Environment::set_path_cache(int i, int j,
                                         std::list<KERNEL::Segment_2> l,
                                         double d) {
  if (i > j) {
    std::swap(i, j);
  };
...
KERNEL::Point_2 Polygon_Environment::get_point_of_segment_on_segment(
  ...
  if (!success) {
    ...
    std::cout << "segment k " << k << " is " << master_polygon->edge(k)
              << std::endl;
...
KERNEL::Point_2
Polygon_Environment::get_point_of_segment_on_segment(KERNEL::Segment_2 s, int k,
                                                     bool &success) {
  if (DEBUG_POLYGON_ENVIRONMENT >= 6) {
    M_INFO1("get_point_of_segment_on_segment seg id %d\n", k);
  ...
  double d;
  d = CGAL::to_double(
      CGAL::squared_distance(s.target(), master_polygon->edge(k)));
  if (DEBUG_POLYGON_ENVIRONMENT >= 4) {
    M_INFO3("get_point_of_segment_on_segment\n");
    std::cout << " segment s " << s << std::endl
              << " edge k " << k << " || " << master_polygon->edge(k)
              << std::endl;
  }
  if (d < _visi_epsilon) {
    success = true;
  ...
  d = CGAL::to_double(
      CGAL::squared_distance(s.source(), master_polygon->edge(k)));
  if (d < _visi_epsilon) {
    success = true;
    return s.source();
  }
...
double Polygon_Environment::get_block_distance(int i, int j) {
  if (!index_bound_check(i) || !index_bound_check(j)) {
    M_ERR("Index is out of bounds");
    return -1;
  }
  double final_dist;
...
bool Polygon_Environment::is_necessary_block(int i, int j) {
  ...
  if (abs(j - i) == 1 || (i == 0 && j == master_polygon->size() - 1) ||
  ...
  for (; it != it_end; it++) {
    int seg_index = get_segment_index_for_point(VisiLibity::Point(
        CGAL::to_double(it->target().x()), CGAL::to_double(it->target().y())));
...
int Polygon_Environment::get_block_cost(int i, int j, double r) {
  fix_index(i);
  fix_index(j);
  i--;
  j--;
...
int Polygon_Environment::get_split_cost(int i, int j, int k, double r) {
  fix_index(i);
  fix_index(j);
  fix_index(k);
  i--;
  j--;
  k--; // indices for cost functions start at 1 but pol env at 0

  if (!index_bound_check(i) || !index_bound_check(j) || !index_bound_check(k)) {
    M_ERR("Index is out of bounds");
    return -1;
  }
  double d1, d2;
  ...
  if (DEBUG_POLYGON_ENVIRONMENT >= 2) {
    std::cout << "get_split_cost " << i << "-" << j << " " << k << " " << cost
              << std::endl;
  }
...
Polygon_Environment::shortest_split_cost(int i, int j, int k, double &cost) {
  ...
  int split_point_index;
  std::list<KERNEL::Segment_2> l =
      shortest_split_costs(i, j, k, cost1, cost2, split_point_index);
  cost = cost1 + cost2;
  return l;
}

bool Polygon_Environment::is_sequence_split(int i, int j, int k) {
...
bool Polygon_Environment::is_sequential(int i, int j) {
...
KERNEL::Point_2 Polygon_Environment::get_other(KERNEL::Point_2 p,
                                               KERNEL::Segment_2 s) {
  if (s.source() == p) {
    return s.target();
  }
...
std::list<KERNEL::Segment_2> Polygon_Environment::shortest_split_costs(
  ...
  if (is_sequential(i, k) && is_sequential(j, k)) {
    ...
    if (i == seque_index) {
      cost1 = 0;
      cost2 = cost;
      split_point_index = return_this.size() - 1;
    } else {
      cost2 = 0;
      cost1 = cost;
      split_point_index = 0;
  ...
  if (p_on_k_from_i == p_on_k_from_j ||
      CGAL::squared_distance(p_on_k_from_i, p_on_k_from_j) < _visi_epsilon) {
  ...
  if (lin.has_on_negative_side(bi1) && lin.has_on_negative_side(bj1)) {
    if (DEBUG_POLYGON_ENVIRONMENT >= 2) {
  ...
  if (lin.has_on_negative_side(bi1)) {
    ...
    l1_best = split_at_point(p_on_k_from_i, i, j, k, cost1, cost2,
                             split_point_index, list_i_k, list_j_k);
    return l1_best;
  }
  if (lin.has_on_negative_side(bj1)) {
    ...
    l1_best = split_at_point(p_on_k_from_j, i, j, k, cost1, cost2,
                             split_point_index, list_i_k, list_j_k);
    return l1_best;
  }
  ...
  while (improvement > 1.0 && tries < max_tries) {
    ...
    double diss = this->shortest_distance_between(master_polygon->edge(k), v,
                                                  closest_point);
    ...
    l1 = split_at_point(v, i, j, k, final_d_i2, final_d_j2, split_point_i_best,
                        list_i_k, list_j_k);
    ...
    improvement = final_d_i2_best + final_d_j2_best - final_d_i2 - final_d_j2;
...
std::list<KERNEL::Segment_2> Polygon_Environment::split_at_point(
    KERNEL::Point_2 v, int i, int j, int k, double &cost1, double &cost2,
    int &split_point_index, std::list<KERNEL::Segment_2> &ll1,
    std::list<KERNEL::Segment_2> &ll2) {
  ll1.clear();
  ll2.clear();
...
KERNEL::Point_2 Polygon_Environment::find_same_angle_point(KERNEL::Point_2 a1,
                                                           KERNEL::Point_2 a2,
                                                           KERNEL::Line_2 l) {
...
bool Polygon_Environment::is_k_in_front(std::list<KERNEL::Segment_2> &l,
                                        int k) {
  bool success = false;
...
void Polygon_Environment::remove_special_vertex_edges() {
...
void Polygon_Environment::remove_special_vertex_direct_visible() {
  int n_segs = seg_vis_graph_type2_vertices.size();
 ...
 */
void Polygon_Environment::process_visibility_polygon(
    int v_index, KERNEL::Point_2 v, VisiLibity::Visibility_Polygon &v_poly,
    bool special_run) {
  int n_segs = seg_vis_graph_type2_vertices.size();
  ...
  if (v_index != n_segs) {
    ...
    (*(this->seg_vis_graph->g))[this->seg_vis_graph_type2_vertices[v_index]]
        .reflexive = v_is_reflexive;
  }
  ...
  for (int i = 0; i < v_poly.n(); i++) {
    ...
    if (endpoint_segment_index == -1 && endpoint_segment_index_next == -1) {
    }
    ...
    if (v_is_reflexive) {
      this->add_edge_to_visibility_graph(
          v_index, 2, v_i_index, 1, dis, CGAL::to_double(v.x()),
          CGAL::to_double(v.y()), CGAL::to_double(closest_p.x()),
    ...
    if (v_is_reflexive || v_index == n_segs) {
      ...
      if (v_i_is_reflexive) {
        ...
        dis = sqrt(dis);
        this->add_edge_to_visibility_graph(
            v_index, 2, v_i_index, 2, dis, CGAL::to_double(v.x()),
            CGAL::to_double(v.y()), CGAL::to_double(v_i.x()),
            CGAL::to_double(v_i.y()));
    ...
    if (special_run && in_air) {
      ...
      if (endpoint_segment_index_next != -1) {
        ...
        int other_edge_index = get_prev_index(endpoint_segment_index_next);

        // test if the previous segment is parallel to edge
        double sqrd_d = CGAL::to_double(
            CGAL::squared_distance(master_polygon->vertex(other_edge_index),
        ...
        if (sqrd_d < _visi_epsilon * _visi_epsilon) {
          ...
          double dis2;
          dis2 = sqrt(CGAL::to_double(seg_to_seg_seg2.squared_length()));
          update_seg_to_seg(n_segs, other_edge_index, seg_to_seg_seg2, dis2);
          this->add_edge_to_visibility_graph(
              v_index, 2, other_edge_index, 2, dis2, CGAL::to_double(v.x()),
              CGAL::to_double(v.y()),
      ...
      if (endpoint_segment_index != -1) {
        ...
        if (sqrd_d < _visi_epsilon * _visi_epsilon) {
          ...
          KERNEL::Segment_2 seg_to_seg_seg2(
              v, master_polygon->vertex(other_edge_index));
          double dis2 = sqrt(CGAL::to_double(seg_to_seg_seg2.squared_length()));
          update_seg_to_seg(n_segs, other_edge_index, seg_to_seg_seg2, dis2);
          this->add_edge_to_visibility_graph(
              v_index, 2, other_edge_index, 2, dis2, CGAL::to_double(v.x()),
              CGAL::to_double(v.y()),
...
void Polygon_Environment::update_seg_to_seg(int i, int j, KERNEL::Segment_2 s,
                                            double d, bool recall) {
  if (DEBUG_POLYGON_ENVIRONMENT_VISI >= 5) {
...
int Polygon_Environment::get_next_index(int i) {
  return (i == (*my_environment)[0].n() - 1) ? 0 : i + 1;
}

int Polygon_Environment::get_prev_index(int i) {
  return (i == 0) ? (*my_environment)[0].n() - 1 : i - 1;
}

std::list<KERNEL::Segment_2>
Polygon_Environment::get_shortest_path(KERNEL::Point_2 v, int j, int k,
                                       double &final_dist) {
  if (DEBUG_POLYGON_ENVIRONMENT >= 2) {
  ...
  p.snap_to_vertices_of(*my_environment, _visi_epsilon);
  VisiLibity::Visibility_Polygon vis_poly;
  vis_poly = VisiLibity::Visibility_Polygon(p, *my_environment, _visi_epsilon);
  int n_segs = seg_vis_graph_type2_vertices.size();
  remove_special_vertex_direct_visible();
  remove_special_vertex_edges();
  Segment_Visibility_Graph::mygraph_t *g = seg_vis_graph->g;
...
Polygon_Environment::get_shortest_path(int i, int j, double &final_dist) {
  ...
  if (i == j) {
    ...
    if (iv < master_polygon->size()) {
      KERNEL::Segment_2 seg(master_polygon->vertex(iv),
                            master_polygon->vertex(iv));
...
void Polygon_Environment::set_edges_type_1_to_infty(int i, int j) {
  Segment_Visibility_Graph::mygraph_t::edge_iterator ei, e_end;
  Segment_Visibility_Graph::mygraph_t *g = seg_vis_graph->g;
...
void Polygon_Environment::add_extra_edges_for(int seg_ind) {
  ...
  if ((*g)[w].reflexive) {
    if (seg_ind == n_segs) {
      double dis = sqrt(pow((*g)[w].p_x - (*g)[special_v].p_x, 2) +
                        pow((*g)[w].p_y - (*g)[special_v].p_y, 2));
      e = this->add_edge_to_visibility_graph(
          seg_ind, 2, (*g)[special_v].segment_index, 2, dis,
          (*g)[special_v].p_x, (*g)[special_v].p_y, (*g)[w].p_x, (*g)[w].p_y);
    } else {
      e = this->add_edge_to_visibility_graph(seg_ind, 2, seg_ind, 1, 0,
                                             (*g)[w].p_x, (*g)[w].p_y,
                                             (*g)[w].p_x, (*g)[w].p_y);
    }
  ...
  if ((*g)[z].reflexive) {
    if (seg_ind == n_segs) {
      double dis = sqrt(pow((*g)[z].p_x - (*g)[special_v].p_x, 2) +
                        pow((*g)[z].p_y - (*g)[special_v].p_y, 2));
      e = this->add_edge_to_visibility_graph(
          seg_ind, 2, next_index, 2, dis, (*g)[special_v].p_x,
          (*g)[special_v].p_y, (*g)[z].p_x, (*g)[z].p_y);
    } else {
      e = this->add_edge_to_visibility_graph(next_index, 2, seg_ind, 1, 0,
                                             (*g)[z].p_x, (*g)[z].p_y,
                                             (*g)[z].p_x, (*g)[z].p_y);
    }
    extra_edges.push_back(e);
...
Polygon_Environment::add_edge_to_visibility_graph(int i, int type_i, int j,
                                                  int type_j, double d,
                                                  double x, double y, double x2,
                                                  double y2) {
  // std::cout
  //    << "  i=" << i << ", " << type_i
  //    << " j=" << j << ", " << type_j
...
int Polygon_Environment::is_endpoint(KERNEL::Point_2 p) {
  Polygon::Vertex_iterator it = master_polygon->vertices_begin();
  int i = 0;
...
KERNEL::Point_2
Polygon_Environment::closest_point_on_line_to(KERNEL::Line_2 l,
                                              KERNEL::Point_2 p) {
  CGAL::Object result;
...
double Polygon_Environment::shortest_distance_between(
    KERNEL::Segment_2 s, KERNEL::Point_2 p, KERNEL::Point_2 &closest_point) {
 ...
 */
int Polygon_Environment::get_segment_index_for_point(VisiLibity::Point p) {
...
void Polygon_Environment::get_visibility_polygon(Segment s) {}

double Polygon_Environment::shortest_path_distance_between(double x1, double y1,
                                                           double x2,
                                                           double y2) {
...
void Polygon_Environment::compute_to_master_distances(int goal) {
  ...
  while (have_unreached && counter < this->size() + 1) {
    ...
    for (int i = 0; i < ((int)this->size()); i++) {
      if ((get_distance2(i, goal) < min_d || min_d == -2) && !done[i] &&
          get_distance2(i, goal) != -1 && get_distance2(i, goal) != 0 &&
          i != goal) {
        min_i = i;
    ...
    if (min_d == -2) {
      ...
      for (int i = 0; i < ((int)this->size()); i++) {
        ...
        if (get_distance(min_i, i) != -1) {
          ...
          if (hop_d < get_distance2(i, goal) || get_distance2(i, goal) == -1) {
            set_distance2(i, goal, hop_d);
            swap_segment(i, goal, min_i);
            if (DEBUG_POLYGONENVIRONMENT >= 3) {
              std::cout << " found better hop to " << i << " at " << hop_d
                        << std::endl;
            }
          }
        }
...
inline double Polygon_Environment::get_distance(int i, int j) {
  if (i < j) {
...
inline void Polygon_Environment::set_distance(int i, int j, double d) {
  if (i < j) {
    distance_mat[j][i] = d;
  } else {
    distance_mat[i][j] = d;
...
inline double Polygon_Environment::get_distance2(int i, int j) {
  if (i < j) {
...
inline void Polygon_Environment::set_distance2(int i, int j, double d) {
  if (i < j) {
    distance_mat2[j][i] = d;
  } else {
    distance_mat2[i][j] = d;
...
inline Segment Polygon_Environment::get_segment(int i, int j, int k) {
  if (i < j) {
...
inline void Polygon_Environment::set_segment(int i, int j, int k, Segment s) {
  if (i < j) {
    segment_mat[j][i][k] = s;
  } else {
    segment_mat[i][j][k] = s;
...
inline void Polygon_Environment::swap_segment(int i, int g, int min_i) {
...
void Polygon_Environment::add_to_master(Polygon *poly, int poly_id, int goal) {
  if (master_polygon->size() == 0) {
    ...
    if (it == poly->vertices_end()) {
      if (DEBUG_POLYGONENVIRONMENT >= 2) {
        std::cout << shortest_s.target() << " shortest target not on poly "
                  << std::endl;
      }
    ...
    if (master_i == master_polygon->vertices_end()) {
      ...
      master_i = master_polygon->find_in_closest(
          shortest_s.source(), 2 * jiggle_factor * jiggle_factor, master_s);
      if (master_i == master_polygon->vertices_end()) {
    ...
    Segment shortest_s2 = polygonization::get_shortest_line(poly_s, master_s);
    if (DEBUG_POLYGONENVIRONMENT >= 2) {
    ...
    if (poly_s.target() == shortest_s.target()) {
      ...
      if (DEBUG_POLYGONENVIRONMENT >= 3) {
        std::cout << " CORRECTING poly_i from " << *master_i << std::endl;
      }
    ...
    if (!do_not_add_p2) {
      if (DEBUG_POLYGONENVIRONMENT >= 3) {
        std::cout << " insert p2 " << p2 << " before " << *it << std::endl;
      }
...
bool Polygon_Environment::is_artificial(Point p) {
  if (find(artificial_points.begin(), artificial_points.end(), p) !=
...
bool Polygon_Environment::is_artificial(Segment s) {
  if (is_artificial(s.target()) && is_artificial(s.source())) {
    return true;
  }
  return false;
}
 ...
 */
bool Polygon_Environment::get_connections_segments(
    Segment &shortest_s, Segment &master_s, Segment &poly_s,
    Polygon::Vertex_iterator &poly_i, Polygon::Vertex_iterator &master_i,
    int poly_id, int goal, Polygon *poly) {
...
bool Polygon_Environment::is_in_polygon(Segment s, Polygon *poly,
                                        Polygon::Vertex_iterator &it,
...
void Polygon_Environment::extract_polygon(Alpha_shape::vertex_handle v) {
  ...
  if (fabs(CGAL::to_double(this->back().area())) <=
      Params::g_min_polygon_area) {
    this->pop_back();
    --(this->n_polygons);
  }
}

void Polygon_Environment::visit_alpha_vertex(
...
bool Polygon_Environment::find_next_regular_vertex(
    Alpha_shape::vertex_handle v, Alpha_shape::vertex_handle &vr) {
...
bool Polygon_Environment::find_next_regular_vertex(
    Alpha_shape::vertex_handle v, Alpha_shape::vertex_handle v_prev,
...
bool Polygon_Environment::find_last(Alpha_shape::vertex_handle v,
                                    Alpha_shape::vertex_handle v_start) {
  Alpha_shape::vertex_handle v_neigh;
  Alpha_shape::edge_circulator e_circ1, e_circ2;
  e_circ1 = A->incident_edges(v);
