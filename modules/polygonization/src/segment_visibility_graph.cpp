    ...
    Segment_Visibility_Graph::vertex goal )
{
    ...
    try {
        astar_goal_visitor<vertex,edge_descriptor> astar_visitor(goal, start,&colormap);
        boost::astar_search
            (*g, start,
...
Segment_Visibility_Graph::get_edge_shortest(vertex v, vertex w)
{
    ...
    for (; ei != ei_end; ++ei)
    {
        ...
        if ( v_target == w ) {
            ...
            //    <<  ":" << (*g)[*ei].p_y2
            //    << std::endl;
            if ( (*g)[*ei].distance < previous_distance 
            || previous_distance == -1 ) {
...
Segment_Visibility_Graph::edge_descriptor
Segment_Visibility_Graph::add_edge(vertex v1, vertex v2, double x, double y, double x2, double y2, double d)
{
    seg_edge p;
    p.p_x = x;
    p.p_y = y;
